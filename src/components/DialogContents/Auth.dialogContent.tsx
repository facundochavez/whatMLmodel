  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>Please log in or register to access the AI features.</DialogDescription>
      </DialogHeader>
      <DialogClose asChild>
        <Button className="w-full gap-2" variant="secondary">
          <Image src="/icons/icon-google.svg" alt="Google Icon" width={20} height={20} />
          Continue with Google
        </Button>
      </DialogClose>
      <div className="flex items-center gap-4 -mb-4">
        <Separator className="flex-grow w-0"></Separator>
        <span className="block text-sm text-center text-muted-foreground">Or</span>
        <Separator className="flex-grow w-0"></Separator>
      </div>
      ...
      <Tabs defaultValue="login" value={isUserRegistering ? 'register' : 'login'}>
        ...
        <TabsContent value="login" tabIndex={-1} className="flex flex-col gap-4 mt-0">
          <LoginForm />
        </TabsContent>
        <TabsContent value="register" tabIndex={-1} className="flex flex-col gap-4 mt-0">
          <RegisterForm />
        </TabsContent>
      </Tabs>
    </DialogContent>
