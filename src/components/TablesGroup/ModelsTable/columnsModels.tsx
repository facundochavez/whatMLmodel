        ...
        >
          ...
          <ul className="w-full min-w-56 [&>li]:w-full [&>li]:text-left [&>li]:flex [&>li]:justify-between [&>li>label]:text-muted-foreground">
            ...
            <li>
              ...
              <span>{row.original.metrics?.accuracy ?? "-"}</span>
            </li>
          </ul>
        </TooltipContent>
      </Tooltip>
...
const ActionsCell: React.FC<{ row: any }> = ({ row }) => {
  ...
  return (
    <Dialog>
      ...
      {isGenerating ? <GenerateCodeDialogContent /> : <PipelineDialogContent />}
    </Dialog>
  );
};
...
const columnsModels = (type: ProblemType): ColumnDef<Model>[] => [
  {
    ...
    cell: ({ row }) => <ModelCell row={row} />, // Usamos el nuevo componente
  },
  {
    id: "actions",
    enableHiding: false,
    cell: ({ row }) => <ActionsCell row={row} />, // Usamos el nuevo componente
  },
];
