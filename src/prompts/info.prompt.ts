...
export const infoPrompt = `Based on the inputDescription describing a dataset and its target variable, generate the next Output in JSON format. Important: keep english for the keys but recognize the language of the InputDescription and use it for the values:
{
  ...
  "info": {
    ...
    "columns": Number: number of columns in the dataset. If the InputDescription doesn’t specify it, invent a reasonable value based on the type of problem,
    "rows": Number: number of rows in the dataset. If the InputDescription doesn’t specify it, invent a reasonable value based on the type of problem,
